function [raster, parm, sigmaParm, x_nodes, y_nodes] = bil_interp_gS ...
...
% Type of delta discretization(1d/2d):  1
...
job_file = strcat('.', par, 'job', par, filename, '_bil.job');
...
fprintf(fid, '%s\n', strcat('.', par, 'data_output', par, filename, '_bil'));
...
if ismac
    job_execution = strcat('./geoSplinter_analysis_macOS < ./job/', filename, '_bil.job');
elseif ispc
    job_execution = strcat('.\geoSplinter_analysis_win < .\job\', filename, '_bil.job');
end
...
[~, raster_tmp, parm_tmp, sigmaParm_tmp, nodes_coord_tmp] = ...
    geoSplinter_noFig (strcat('.', par, 'data_output', par, filename, '_bil'), 'bil');
